function [Fd, Fc, H2, P, solver_stats] = h2syn_nominal(sysD, sysC, ny, nu, L0, backoff)
...
for i = 2:nL
    ...
    LMI = [ Qi               Ji              A*Y+Bu*M       A+Bu*N*Cy      Bw+Bu*N*Dyw  ;
            Ji'              Hi              K              X*A+L*Cy       X*Bw+L*Dyw   ;
            Y'*A'+M'*Bu'     K'              Y+Y'-Qi        eye(nx)+S'-Ji  zeros(nx,nw) ;
            A'+Cy'*N'*Bu'    A'*X'+Cy'*L'    eye(nx)+S-Ji'  X+X'-Hi        zeros(nx,nw) ;
            Bw'+Dyw'*N'*Bu'  Bw'*X'+Dyw'*L'  zeros(nw,nx)   zeros(nw,nx)   eye(nw)      ];
    TRC = [ Wi                 Cz*Y+Dzu*M     Cz+Dzu*N*Cy    Dzw+Dzu*N*Dyw ;
            Y'*Cz'+M'*Dzu'     Y+Y'-Qi        eye(nx)+S'-Ji  zeros(nx,nw)  ;
            Cz'+Cy'*N'*Dzu'    eye(nx)+S-Ji'  X+X'-Hi        zeros(nx,nw)  ;
            Dzw'+Dyw'*N'*Dzu'  zeros(nw,nx)   zeros(nw,nx)   eye(nw)       ];
    cost = cost + trace(Wi);
